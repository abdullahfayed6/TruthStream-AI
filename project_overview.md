...
| Feature | Status |
|---|---|
| DistilBERT / RoBERTa weights | ✅ `ml/models/distilbert-fakenews/` — hamzab/roberta-fake-news-classification (475 MB) |
| OpenAI explanation layer | ✅ `api/routers/explain.py` — GPT-4o-mini explain + summarize endpoints |
| FastAPI backend | ✅ `api/` — `/articles`, `/stats`, `/stats/timeline`, `/explain`, `/summarize` |
| Streamlit dashboard | ✅ `dashboard/app.py` — Overview, Feed, Inspect (GPT), About |
| Gold Parquet + MongoDB | ✅ Flowing — 103 articles scored and persisted |
...
## Start App (after Docker stack is up)
